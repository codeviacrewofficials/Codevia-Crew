          ...
          <section className="w-full max-w-6xl mx-auto mt-8">
            <div className="flex border-b border-[#E5E7EB] dark:border-gray-700/80">
              {SERVICE_TABS.map((t) => (
                <div
                  key={t.key}
                  className={`flex items-center px-8 py-4 cursor-pointer transition-all duration-200 font-inter ${activeTab === t.key ? 'border-b-2 border-blue-500' : 'border-b-2 border-transparent'}`}
                  style={{width: t.key === 'web' ? 185 : t.key === 'android' ? 210 : 268}}
                  onClick={() => setActiveTab(t.key)}
                >
                  <span className={`text-2xl mr-3 ${activeTab === t.key ? 'text-blue-600 dark:text-blue-500' : 'text-gray-400'}`}>{t.icon}</span>
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-col gap-4 mb-6">
                  {tab.content.benefits.map((b, i) => (
                    <div key={b.title} className="bg-[#F3F4F6] dark:bg-[#1F2937] rounded-lg px-6 py-4 flex flex-col md:flex-row md:items-center justify-between">
                      <div>
                        <span className="text-[#232B3A] dark:text-white font-semibold font-inter text-base block mb-1 md:mb-0">{b.title}</span>
                        <span className="block text-xs text-[#6B7280] dark:text-[#9CA3AF] font-inter md:hidden">{b.desc}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="hidden md:block text-xs text-[#6B7280] dark:text-[#9CA3AF] font-inter mr-4">{b.desc}</span>
                        <span className="inline-block ml-1">
                          <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M7 8l3 3 3-3" stroke="#232B3A" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="dark:stroke-white" />
                          </svg>
                        </span>
                      </div>
                    </div>
